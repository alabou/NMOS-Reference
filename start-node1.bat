...
rem Windows equivalent of start-node1.sh.
rem
rem Configuration C (mTLS + OAuth 2.0) -- TR-10-SEC 12.3 RAAM=2. Serves the Controller UI on 5050.
rem
rem Usage:
rem   start-node1.bat [as-host] [as-port] [rds-host] [rds-port]
...
pushd "%SCRIPT_DIR%" >nul || (
  >&2 echo start-node1.bat: cannot enter "%SCRIPT_DIR%"
  exit /b 1
)
...
>&2 echo start-node1.bat: unknown argument %ARG%
set "EXIT_CODE=64"
goto done

:options_done
if not "%NAP%"=="2" (
  >&2 echo start-node1.bat: Config C ^(RAAM=2^) pins NAP=2; got --nap=%NAP%
  set "EXIT_CODE=64"
  goto done
)
...
set "CERT_PROBE=pem\ExampleDeviceServer.ABC.SNX00001.chain.pem"
if defined IPMX_CERT_ROOT (
  set "CERT_ROOT=%IPMX_CERT_ROOT%"
) else if exist "%SCRIPT_DIR%Certificates\build.0\pem\ExampleDeviceServer.ABC.SNX00001.chain.pem" (
  ...
  set "CERT_ROOT=%SCRIPT_DIR%..\Certificates"
  >&2 echo start-node1.bat: %CERT_PROBE% is not in this checkout - using the workspace PKI.
) else (
  >&2 echo start-node1.bat: missing build.0\%CERT_PROBE%
...
set "NODE_CERT_ARGS="
if not "%TCT%"=="1" set "NODE_CERT_ARGS=--nodeCertificate "%CERTS%\pem\ExampleDeviceServer.ABC.SNX00001.chain.pem" --nodeKey "%CERTS%\key\ExampleDeviceServer.ABC.SNX00001.key""
if not "%TCT%"=="0" set "NODE_CERT_ARGS=%NODE_CERT_ARGS% --nodeCertificate "%CERTS%\pem\ExampleDeviceServer.ABC.SNX00001.chain.ec.pem" --nodeKey "%CERTS%\key\ExampleDeviceServer.ABC.SNX00001.ec.key""
...
set "RDS_CLIENT_ARGS="
if not "%TCT%"=="1" set "NODE_CLIENT_ARGS=--nodeClientCertificate "%CERTS%\pem\ExampleDeviceClient.ABC.SNX00001.chain.pem" --nodeClientKey "%CERTS%\key\ExampleDeviceClient.ABC.SNX00001.key""
if not "%TCT%"=="0" set "NODE_CLIENT_ARGS=%NODE_CLIENT_ARGS% --nodeClientCertificate "%CERTS%\pem\ExampleDeviceClient.ABC.SNX00001.chain.ec.pem" --nodeClientKey "%CERTS%\key\ExampleDeviceClient.ABC.SNX00001.ec.key""
if not "%TCT%"=="1" set "RDS_CLIENT_ARGS=--rdsClientCertificate "%CERTS%\pem\ExampleDeviceClient.ABC.SNX00001.chain.pem" --rdsClientKey "%CERTS%\key\ExampleDeviceClient.ABC.SNX00001.key""
if not "%TCT%"=="0" set "RDS_CLIENT_ARGS=%RDS_CLIENT_ARGS% --rdsClientCertificate "%CERTS%\pem\ExampleDeviceClient.ABC.SNX00001.chain.ec.pem" --rdsClientKey "%CERTS%\key\ExampleDeviceClient.ABC.SNX00001.ec.key""
...
echo Node SNX00001: Config C ^(mTLS + OAuth 2.0^), NAP=%NAP% RAP=%RAP% OAIM=%OAIM% TCT=%TCT%
...
:require_root
if exist "%~1" exit /b 0
>&2 echo start-node1.bat: missing "%~1"
>&2 echo   Set IPMX_CERT_ROOT to a Certificates tree that carries it.
exit /b 1
...
if errorlevel 1 (
  >&2 echo start-node1.bat: cannot build a CA bundle from "%CERTS%"
  exit /b 1
)
exit /b 0
...
:require_port_bad
>&2 echo start-node1.bat: %PORT_LABEL% must be a whole number between %PORT_MIN% and %PORT_MAX%, got "%PORT_VALUE%"
exit /b 1
