...
if errorlevel 1 (
  >&2 echo start-node1.bat: invalid registration port "%RDS_REG_PORT%"
  set "EXIT_CODE=64"
  goto done
)
...
copy /b "%CERTS%\ExampleRootCA.pem" + "%CERTS%\ExampleRootCA.ec.pem" "%CA%" >nul || (
  >&2 echo start-node1.bat: cannot build CA bundle from "%CERTS%"
  set "EXIT_CODE=1"
  goto done
)
...
call :find_python
if errorlevel 1 (
  >&2 echo start-node1.bat: Python 3.12 or newer was not found.
