...
/// `NWebSocketReceiverTransportParams`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NWebSocketReceiverTransportParams {
...
impl NWebSocketReceiverTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NWebSocketReceiverTransportParams",
            ));
        };
