...
use crate::generated::nreceiver_data_capabilities::NReceiverDataCapabilities;
...
pub struct NReceiverData {
    ...
    /// `caps`. Required.
    #[serde(rename = "caps")]
    pub caps: NReceiverDataCapabilities,
}
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NReceiverData",
            ));
        };
        ...
        let caps = match doc.get("caps") {
            Some(v) => Some(NReceiverDataCapabilities::decode(v)?),
            None => None,
        };
