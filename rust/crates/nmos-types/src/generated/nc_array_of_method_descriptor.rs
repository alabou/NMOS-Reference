...
use crate::generated::nc_method_descriptor::NcMethodDescriptor;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NcArrayOfMethodDescriptor",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcMethodDescriptor::decode(item)?);
        }
        Ok(Self(out))
    }
}
