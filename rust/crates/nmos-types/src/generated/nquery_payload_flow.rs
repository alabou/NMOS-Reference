...
use crate::generated::nquery_web_socket_grain_flow::NQueryWebSocketGrainFlow;
...
pub struct NQueryPayloadFlow {
    ...
    /// `grain`. Required.
    #[serde(rename = "grain")]
    pub grain: NQueryWebSocketGrainFlow,
}

impl NQueryPayloadFlow {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryPayloadFlow",
            ));
        };
        ...
        let grain = match doc.get("grain") {
            Some(v) => Some(NQueryWebSocketGrainFlow::decode(v)?),
            None => None,
        };
