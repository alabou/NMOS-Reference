...
use crate::generated::nusb_transport_constraints::NUsbTransportConstraints;
...
/// `NArrayOfUsbTransportConstraints`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfUsbTransportConstraints(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NUsbTransportConstraints>,
);

impl NArrayOfUsbTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfUsbTransportConstraints",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NUsbTransportConstraints::decode(item)?);
        }
        Ok(Self(out))
    }
}
