...
use crate::generated::nreceiver_mux_capabilities::NReceiverMuxCapabilities;
...
pub struct NReceiverMux {
    ...
    /// `caps`. Required.
    #[serde(rename = "caps")]
    pub caps: NReceiverMuxCapabilities,
}
...
impl NReceiverMux {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data("expected JSON object for NReceiverMux"));
        };
        ...
        let caps = match doc.get("caps") {
            Some(v) => Some(NReceiverMuxCapabilities::decode(v)?),
            None => None,
        };
