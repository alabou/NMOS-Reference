...
use crate::generated::nweb_socket_transport_constraints::NWebSocketTransportConstraints;
...
/// `NArrayOfWebSocketTransportConstraints`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfWebSocketTransportConstraints(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NWebSocketTransportConstraints>,
);

impl NArrayOfWebSocketTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfWebSocketTransportConstraints",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NWebSocketTransportConstraints::decode(item)?);
        }
        Ok(Self(out))
    }
}
