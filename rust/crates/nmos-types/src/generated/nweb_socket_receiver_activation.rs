...
use crate::generated::narray_of_web_socket_receiver_transport_params::NArrayOfWebSocketReceiverTransportParams;
...
pub struct NWebSocketReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfWebSocketReceiverTransportParams>,
}

impl NWebSocketReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NWebSocketReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfWebSocketReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NWebSocketReceiverActivation"
                )));
            }
        }
