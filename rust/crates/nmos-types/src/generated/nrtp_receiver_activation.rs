...
use crate::generated::narray_of_rtp_receiver_transport_params::NArrayOfRtpReceiverTransportParams;
...
pub struct NRtpReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfRtpReceiverTransportParams>,
}

impl NRtpReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtpReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfRtpReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NRtpReceiverActivation"
                )));
            }
        }
