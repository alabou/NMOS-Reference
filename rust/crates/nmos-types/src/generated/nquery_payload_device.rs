...
use crate::generated::nquery_web_socket_grain_device::NQueryWebSocketGrainDevice;
...
pub struct NQueryPayloadDevice {
    /// `grain_type`. Required.
    ...
    /// `grain`. Required.
    #[serde(rename = "grain")]
    pub grain: NQueryWebSocketGrainDevice,
}

impl NQueryPayloadDevice {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryPayloadDevice",
            ));
        };
        ...
        let grain = match doc.get("grain") {
            Some(v) => Some(NQueryWebSocketGrainDevice::decode(v)?),
            None => None,
        };
