...
use crate::generated::nconstraint_set::NConstraintSet;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfConstraintSet",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NConstraintSet::decode(item)?);
        }
        Ok(Self(out))
    }
}
