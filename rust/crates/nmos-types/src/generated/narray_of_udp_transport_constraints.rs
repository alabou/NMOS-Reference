...
use crate::generated::nudp_transport_constraints::NUdpTransportConstraints;
...
/// `NArrayOfUdpTransportConstraints`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfUdpTransportConstraints(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NUdpTransportConstraints>,
);

impl NArrayOfUdpTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfUdpTransportConstraints",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NUdpTransportConstraints::decode(item)?);
        }
        Ok(Self(out))
    }
}
