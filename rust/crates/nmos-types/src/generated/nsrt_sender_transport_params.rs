...
/// `NSrtSenderTransportParams`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NSrtSenderTransportParams {
    ...
    pub destination_ip: Option<Nullable<String>>,
...
impl NSrtSenderTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NSrtSenderTransportParams",
            ));
        };
