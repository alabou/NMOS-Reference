...
use crate::generated::nc_command::NcCommand;
...
impl NcArrayOfCommand {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data("expected array for NcArrayOfCommand"));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcCommand::decode(item)?);
        }
        Ok(Self(out))
    }
}
