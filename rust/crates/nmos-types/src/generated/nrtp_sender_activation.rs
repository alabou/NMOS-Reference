...
use crate::generated::narray_of_rtp_sender_transport_params::NArrayOfRtpSenderTransportParams;
...
pub struct NRtpSenderActivation {
    ...
    pub transport_params: Option<NArrayOfRtpSenderTransportParams>,
}

impl NRtpSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtpSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfRtpSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NRtpSenderActivation"
                )));
            }
        }
