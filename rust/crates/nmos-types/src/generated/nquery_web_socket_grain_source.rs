...
use crate::generated::narray_of_query_web_socket_grain_data_source::NArrayOfQueryWebSocketGrainDataSource;
...
/// `NQueryWebSocketGrainSource`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NQueryWebSocketGrainSource {
    /// `type`. Required.
    #[serde(rename = "type")]
    pub r#type: String,
    /// `topic`. Required.
    ...
    /// `data`. Required.
    #[serde(rename = "data")]
    pub data: NArrayOfQueryWebSocketGrainDataSource,
}

impl NQueryWebSocketGrainSource {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryWebSocketGrainSource",
            ));
        };
        ...
        let data = match doc.get("data") {
            Some(v) => Some(NArrayOfQueryWebSocketGrainDataSource::decode(v)?),
            None => None,
        };
