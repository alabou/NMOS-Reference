...
use crate::generated::mv_alert_capability_descriptor::MvAlertCapabilityDescriptor;
...
/// `MvArrayOfAlertCapabilityDescriptor`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct MvArrayOfAlertCapabilityDescriptor(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<MvAlertCapabilityDescriptor>,
);

impl MvArrayOfAlertCapabilityDescriptor {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for MvArrayOfAlertCapabilityDescriptor",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(MvAlertCapabilityDescriptor::decode(item)?);
        }
        Ok(Self(out))
    }
}
