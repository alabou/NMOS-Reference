...
use crate::generated::narray_of_udp_sender_transport_params::NArrayOfUdpSenderTransportParams;
...
pub struct NUdpSenderActivation {
    ...
    pub transport_params: Option<NArrayOfUdpSenderTransportParams>,
}

impl NUdpSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NUdpSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfUdpSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NUdpSenderActivation"
                )));
            }
        }
