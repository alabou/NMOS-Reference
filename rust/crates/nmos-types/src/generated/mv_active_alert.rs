...
use crate::generated::mv_array_of_event_counter::MvArrayOfEventCounter;
...
pub struct MvActiveAlert {
    ...
    /// `eventCounters`. Required.
    #[serde(rename = "eventCounters")]
    pub event_counters: MvArrayOfEventCounter,
}
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for MvActiveAlert",
            ));
        };
        ...
        let event_counters = match doc.get("eventCounters") {
            Some(v) => Some(MvArrayOfEventCounter::decode(v)?),
            None => None,
        };
        ...
        let event_counters = event_counters
            .ok_or_else(|| Error::invalid_object("missing required member EventCounters"))?;
