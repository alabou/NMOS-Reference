...
use crate::generated::nquery_web_socket_grain_data_receiver::NQueryWebSocketGrainDataReceiver;
...
/// `NArrayOfQueryWebSocketGrainDataReceiver`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfQueryWebSocketGrainDataReceiver(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NQueryWebSocketGrainDataReceiver>,
);

impl NArrayOfQueryWebSocketGrainDataReceiver {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfQueryWebSocketGrainDataReceiver",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NQueryWebSocketGrainDataReceiver::decode(item)?);
        }
        Ok(Self(out))
    }
}
