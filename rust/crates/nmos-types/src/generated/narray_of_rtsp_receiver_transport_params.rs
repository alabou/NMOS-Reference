...
use crate::generated::nrtsp_receiver_transport_params::NRtspReceiverTransportParams;
...
/// `NArrayOfRtspReceiverTransportParams`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfRtspReceiverTransportParams(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NRtspReceiverTransportParams>,
);

impl NArrayOfRtspReceiverTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfRtspReceiverTransportParams",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NRtspReceiverTransportParams::decode(item)?);
        }
        Ok(Self(out))
    }
}
