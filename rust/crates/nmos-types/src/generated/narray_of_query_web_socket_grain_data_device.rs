...
use crate::generated::nquery_web_socket_grain_data_device::NQueryWebSocketGrainDataDevice;
...
/// `NArrayOfQueryWebSocketGrainDataDevice`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfQueryWebSocketGrainDataDevice(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NQueryWebSocketGrainDataDevice>,
);

impl NArrayOfQueryWebSocketGrainDataDevice {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfQueryWebSocketGrainDataDevice",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NQueryWebSocketGrainDataDevice::decode(item)?);
        }
        Ok(Self(out))
    }
}
