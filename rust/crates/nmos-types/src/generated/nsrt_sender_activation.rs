...
use crate::generated::narray_of_srt_sender_transport_params::NArrayOfSrtSenderTransportParams;
...
pub struct NSrtSenderActivation {
    ...
    pub transport_params: Option<NArrayOfSrtSenderTransportParams>,
}

impl NSrtSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NSrtSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfSrtSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NSrtSenderActivation"
                )));
            }
        }
