...
/// `NRtpReceiverTransportParams`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NRtpReceiverTransportParams {
    ...
    /// `multicast_ip`. Optional, so absent means the member was not present.
    #[serde(rename = "multicast_ip", skip_serializing_if = "Option::is_none")]
    pub multicast_ip: Option<Nullable<String>>,
...
impl NRtpReceiverTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtpReceiverTransportParams",
            ));
        };
        ...
        let multicast_ip = match doc.get("multicast_ip") {
            Some(v) => Some(decode::nullable_string(v)?),
            None => None,
        };
