...
/// `NRtpSenderTransportParams`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NRtpSenderTransportParams {
    ...
    /// `rtcp_enabled`. Optional, so absent means the member was not present.
    #[serde(rename = "rtcp_enabled", skip_serializing_if = "Option::is_none")]
    pub rtcp_enabled: Option<bool>,
    /// `rtcp_destination_ip`. Optional, so absent means the member was not present.
    #[serde(
        rename = "rtcp_destination_ip",
        skip_serializing_if = "Option::is_none"
    )]
    pub rtcp_destination_ip: Option<String>,
    /// `rtcp_destination_port`. Optional, so absent means the member was not present.
    #[serde(
        rename = "rtcp_destination_port",
        skip_serializing_if = "Option::is_none"
    )]
    pub rtcp_destination_port: Option<Nullable<Value>>,
    /// `rtcp_source_port`. Optional, so absent means the member was not present.
    #[serde(rename = "rtcp_source_port", skip_serializing_if = "Option::is_none")]
    pub rtcp_source_port: Option<Nullable<Value>>,
    /// `rtp_enabled`. Optional, so absent means the member was not present.
    #[serde(rename = "rtp_enabled", skip_serializing_if = "Option::is_none")]
...
impl NRtpSenderTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtpSenderTransportParams",
            ));
        };
        ...
        let rtcp_enabled = match doc.get("rtcp_enabled") {
            Some(v) => Some(decode::bool(v)?),
            None => None,
        };
        let rtcp_destination_ip = match doc.get("rtcp_destination_ip") {
            Some(v) => decode::string(v)?,
            None => None,
        };
        let rtcp_destination_port = doc.get("rtcp_destination_port").map(decode::null_value);
        let rtcp_source_port = doc.get("rtcp_source_port").map(decode::null_value);
        let rtp_enabled = match doc.get("rtp_enabled") {
            Some(v) => Some(decode::bool(v)?),
            None => None,
        };
        ...
        if let Some(v) = &rtcp_destination_port {
            validators::check_auto_port(v.as_json())?;
        }
        if let Some(v) = &rtcp_source_port {
            validators::check_auto_port(v.as_json())?;
        }
