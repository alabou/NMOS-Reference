...
use crate::generated::narray_of_rtsp_sender_transport_params::NArrayOfRtspSenderTransportParams;
...
pub struct NRtspSenderActivation {
    ...
    pub transport_params: Option<NArrayOfRtspSenderTransportParams>,
}

impl NRtspSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtspSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfRtspSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NRtspSenderActivation"
                )));
            }
        }
