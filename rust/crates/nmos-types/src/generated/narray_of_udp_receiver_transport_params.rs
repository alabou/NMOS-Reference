...
use crate::generated::nudp_receiver_transport_params::NUdpReceiverTransportParams;
...
/// `NArrayOfUdpReceiverTransportParams`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfUdpReceiverTransportParams(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NUdpReceiverTransportParams>,
);

impl NArrayOfUdpReceiverTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfUdpReceiverTransportParams",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NUdpReceiverTransportParams::decode(item)?);
        }
        Ok(Self(out))
    }
}
