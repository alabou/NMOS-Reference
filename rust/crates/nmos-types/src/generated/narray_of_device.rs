...
impl NArrayOfDevice {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data("expected array for NArrayOfDevice"));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NDevice::decode(item)?);
        }
        Ok(Self(out))
    }
}
