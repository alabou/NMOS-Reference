...
pub struct NcSubscriptionMessage {
    /// `messageType`. Required.
    #[serde(rename = "messageType")]
    pub message_type: i64,
    /// `subscriptions`. Required.
    #[serde(rename = "subscriptions")]
    pub subscriptions: Nullable<Value>,
}

impl NcSubscriptionMessage {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NcSubscriptionMessage",
            ));
        };
        ...
        let subscriptions = doc.get("subscriptions").map(decode::null_value);
        ...
        // present.
        validators::check_subscription_message_type(message_type)?;
