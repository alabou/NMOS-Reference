...
use crate::generated::nc_notification::NcNotification;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NcArrayOfNotification",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcNotification::decode(item)?);
        }
        Ok(Self(out))
    }
}
