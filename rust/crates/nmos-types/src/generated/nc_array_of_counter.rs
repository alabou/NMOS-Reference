...
use crate::generated::nc_counter::NcCounter;
...
impl NcArrayOfCounter {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data("expected array for NcArrayOfCounter"));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcCounter::decode(item)?);
        }
        Ok(Self(out))
    }
}
