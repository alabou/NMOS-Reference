...
use crate::generated::nc_block_member_descriptor::NcBlockMemberDescriptor;
...
/// `NcArrayOfBlockMemberDescriptor`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NcArrayOfBlockMemberDescriptor(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NcBlockMemberDescriptor>,
);

impl NcArrayOfBlockMemberDescriptor {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NcArrayOfBlockMemberDescriptor",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcBlockMemberDescriptor::decode(item)?);
        }
        Ok(Self(out))
    }
}
