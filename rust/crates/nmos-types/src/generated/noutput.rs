...
use crate::generated::noutput_status::NOutputStatus;
...
pub struct NOutput {
    ...
    /// `device_id`. Optional, so absent means the member was not present.
...
impl NOutput {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data("expected JSON object for NOutput"));
        };
        ...
        let status = match doc.get("status") {
            Some(v) => Some(NOutputStatus::decode(v)?),
            None => None,
        };
