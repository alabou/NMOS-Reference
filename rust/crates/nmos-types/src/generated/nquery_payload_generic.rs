...
use crate::generated::nquery_web_socket_grain_generic::NQueryWebSocketGrainGeneric;
...
/// `NQueryPayloadGeneric`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NQueryPayloadGeneric {
    /// `grain_type`. Required.
    ...
    /// `grain`. Required.
    #[serde(rename = "grain")]
    pub grain: NQueryWebSocketGrainGeneric,
}

impl NQueryPayloadGeneric {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryPayloadGeneric",
            ));
        };
        ...
        let grain = match doc.get("grain") {
            Some(v) => Some(NQueryWebSocketGrainGeneric::decode(v)?),
            None => None,
        };
