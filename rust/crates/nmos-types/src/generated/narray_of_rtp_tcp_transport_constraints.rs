...
use crate::generated::nrtp_tcp_transport_constraints::NRtpTcpTransportConstraints;
...
/// `NArrayOfRtpTcpTransportConstraints`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfRtpTcpTransportConstraints(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NRtpTcpTransportConstraints>,
);

impl NArrayOfRtpTcpTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfRtpTcpTransportConstraints",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NRtpTcpTransportConstraints::decode(item)?);
        }
        Ok(Self(out))
    }
}
