...
pub struct NSenderSubscription {
    /// `receiver_id`. Required.
    #[serde(rename = "receiver_id")]
    pub receiver_id: Nullable<String>,
    /// `active`. Required.
    #[serde(rename = "active")]
    pub active: bool,
}

impl NSenderSubscription {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NSenderSubscription",
            ));
        };
        ...
        let receiver_id = receiver_id
            .ok_or_else(|| Error::invalid_object("missing required member ReceiverId"))?;
        let active =
        ...
        // present.
        validators::check_resource_id_nullable_string(receiver_id.as_option().map(String::as_str))?;
