...
use crate::generated::narray_of_mqtt_receiver_transport_params::NArrayOfMqttReceiverTransportParams;
...
pub struct NMqttReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfMqttReceiverTransportParams>,
}

impl NMqttReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NMqttReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfMqttReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NMqttReceiverActivation"
                )));
            }
        }
