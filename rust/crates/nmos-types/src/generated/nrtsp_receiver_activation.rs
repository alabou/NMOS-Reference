...
use crate::generated::narray_of_rtsp_receiver_transport_params::NArrayOfRtspReceiverTransportParams;
...
pub struct NRtspReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfRtspReceiverTransportParams>,
}

impl NRtspReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtspReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfRtspReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NRtspReceiverActivation"
                )));
            }
        }
