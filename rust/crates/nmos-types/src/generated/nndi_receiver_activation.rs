...
use crate::generated::narray_of_ndi_receiver_transport_params::NArrayOfNdiReceiverTransportParams;
...
pub struct NNdiReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfNdiReceiverTransportParams>,
}

impl NNdiReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NNdiReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfNdiReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NNdiReceiverActivation"
                )));
            }
        }
