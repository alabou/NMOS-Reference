...
use crate::generated::narray_of_srt_receiver_transport_params::NArrayOfSrtReceiverTransportParams;
...
pub struct NSrtReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfSrtReceiverTransportParams>,
}

impl NSrtReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NSrtReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfSrtReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NSrtReceiverActivation"
                )));
            }
        }
