...
use crate::generated::narray_of_udp_receiver_transport_params::NArrayOfUdpReceiverTransportParams;
...
pub struct NUdpReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfUdpReceiverTransportParams>,
}

impl NUdpReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NUdpReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfUdpReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NUdpReceiverActivation"
                )));
            }
        }
