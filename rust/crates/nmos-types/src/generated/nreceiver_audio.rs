...
use crate::generated::nreceiver_audio_capabilities::NReceiverAudioCapabilities;
...
pub struct NReceiverAudio {
    ...
    /// `caps`. Required.
    #[serde(rename = "caps")]
    pub caps: NReceiverAudioCapabilities,
}
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NReceiverAudio",
            ));
        };
        ...
        let caps = match doc.get("caps") {
            Some(v) => Some(NReceiverAudioCapabilities::decode(v)?),
            None => None,
        };
