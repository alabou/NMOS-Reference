...
use crate::generated::narray_of_usb_receiver_transport_params::NArrayOfUsbReceiverTransportParams;
...
pub struct NUsbReceiverActivation {
    ...
    pub transport_params: Option<NArrayOfUsbReceiverTransportParams>,
}

impl NUsbReceiverActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NUsbReceiverActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfUsbReceiverTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NUsbReceiverActivation"
                )));
            }
        }
