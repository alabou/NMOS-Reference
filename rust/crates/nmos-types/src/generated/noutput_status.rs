...
pub struct NOutputStatus {
    /// `state`. Required.
    #[serde(rename = "state")]
    pub state: EnumId,
    /// `debug`. Optional, so absent means the member was not present.
    #[serde(rename = "debug", skip_serializing_if = "Option::is_none")]
    pub debug: Option<Nullable<String>>,
}
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NOutputStatus",
            ));
        };
        ...
        let state = state.ok_or_else(|| Error::invalid_object("missing required member State"))?;
        ...
        // present.
        validators::check_output_status_state(&state)?;
