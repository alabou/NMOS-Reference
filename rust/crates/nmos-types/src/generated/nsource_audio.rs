...
use crate::generated::narray_of_audio_channel::NArrayOfAudioChannel;
...
pub struct NSourceAudio {
    ...
    pub channels: NArrayOfAudioChannel,
}
...
impl NSourceAudio {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data("expected JSON object for NSourceAudio"));
        };
        ...
        let channels = match doc.get("channels") {
            Some(v) => Some(NArrayOfAudioChannel::decode(v)?),
            None => None,
        };
        ...
        let channels =
            channels.ok_or_else(|| Error::invalid_object("missing required member Channels"))?;
        ...
        validators::check_audio_channels(
            channels
                .0
                .iter()
                .map(|e| e.symbol.as_ref().map(nmos_json::EnumId::as_str)),
        )?;
