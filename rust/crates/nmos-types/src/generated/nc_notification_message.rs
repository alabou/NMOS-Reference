...
use crate::generated::nc_array_of_notification::NcArrayOfNotification;
...
pub struct NcNotificationMessage {
    /// `messageType`. Required.
    #[serde(rename = "messageType")]
    pub message_type: i64,
    /// `notifications`. Required.
    #[serde(rename = "notifications")]
    pub notifications: NcArrayOfNotification,
}

impl NcNotificationMessage {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NcNotificationMessage",
            ));
        };
        ...
        let notifications = match doc.get("notifications") {
            Some(v) => Some(NcArrayOfNotification::decode(v)?),
            None => None,
        };
        ...
        let notifications = notifications
            .ok_or_else(|| Error::invalid_object("missing required member Notifications"))?;
        ...
        // present.
        validators::check_notification_message_type(message_type)?;
