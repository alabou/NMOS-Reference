...
use crate::generated::narray_of_web_socket_sender_transport_params::NArrayOfWebSocketSenderTransportParams;
...
pub struct NWebSocketSenderActivation {
    ...
    pub transport_params: Option<NArrayOfWebSocketSenderTransportParams>,
}

impl NWebSocketSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NWebSocketSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfWebSocketSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NWebSocketSenderActivation"
                )));
            }
        }
