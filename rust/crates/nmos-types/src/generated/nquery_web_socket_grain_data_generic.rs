    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryWebSocketGrainDataGeneric",
            ));
        };
        ...
        let pre = doc.get("pre").map(decode::raw_json);
        let post = doc.get("post").map(decode::raw_json);
