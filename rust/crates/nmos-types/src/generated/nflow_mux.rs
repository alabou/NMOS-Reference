...
pub struct NFlowMux {
    ...
    /// `urn:x-matrox:video_layers`. Optional, so absent means the member was not present.
    #[serde(
        rename = "urn:x-matrox:video_layers",
        skip_serializing_if = "Option::is_none"
    )]
    pub video_layers: Option<i64>,
    /// `urn:x-matrox:audio_layers`. Optional, so absent means the member was not present.
    #[serde(
        rename = "urn:x-matrox:audio_layers",
        skip_serializing_if = "Option::is_none"
    )]
    pub audio_layers: Option<i64>,
    /// `urn:x-matrox:data_layers`. Optional, so absent means the member was not present.
    #[serde(
        rename = "urn:x-matrox:data_layers",
        skip_serializing_if = "Option::is_none"
    )]
    pub data_layers: Option<i64>,
}
...
impl NFlowMux {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data("expected JSON object for NFlowMux"));
        };
        ...
        let video_layers = match doc.get("urn:x-matrox:video_layers") {
            Some(v) => Some(decode::int(v)?),
            None => None,
        };
        let audio_layers = match doc.get("urn:x-matrox:audio_layers") {
            Some(v) => Some(decode::int(v)?),
            None => None,
        };
        let data_layers = match doc.get("urn:x-matrox:data_layers") {
            Some(v) => Some(decode::int(v)?),
            None => None,
        };
