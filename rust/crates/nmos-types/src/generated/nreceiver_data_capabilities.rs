...
/// `NReceiverDataCapabilities`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NReceiverDataCapabilities {
    ...
    /// `event_types`. Optional, so absent means the member was not present.
    #[serde(rename = "event_types", skip_serializing_if = "Option::is_none")]
    pub event_types: Option<Vec<EnumId>>,
...
impl NReceiverDataCapabilities {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NReceiverDataCapabilities",
            ));
        };
        ...
        let event_types = match doc.get("event_types") {
            Some(v) => Some(decode::array_of_enum(v)?),
            None => None,
        };
        ...
        if let Some(v) = &media_types {
            validators::check_data_media_types(v.iter().map(nmos_json::EnumId::as_str))?;
        }
        if let Some(v) = &event_types {
            validators::check_data_event_types(v.len())?;
        }
