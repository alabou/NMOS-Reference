...
use crate::generated::nc_class_descriptor::NcClassDescriptor;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NcArrayOfClassDescriptor",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcClassDescriptor::decode(item)?);
        }
        Ok(Self(out))
    }
}
