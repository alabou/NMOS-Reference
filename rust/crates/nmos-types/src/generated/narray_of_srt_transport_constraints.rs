...
use crate::generated::nsrt_transport_constraints::NSrtTransportConstraints;
...
/// `NArrayOfSrtTransportConstraints`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfSrtTransportConstraints(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NSrtTransportConstraints>,
);

impl NArrayOfSrtTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfSrtTransportConstraints",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NSrtTransportConstraints::decode(item)?);
        }
        Ok(Self(out))
    }
}
