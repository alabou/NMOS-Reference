...
use crate::generated::narray_of_query_web_socket_grain_data_flow::NArrayOfQueryWebSocketGrainDataFlow;
...
/// `NQueryWebSocketGrainFlow`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NQueryWebSocketGrainFlow {
    /// `type`. Required.
    #[serde(rename = "type")]
    pub r#type: String,
    /// `topic`. Required.
    ...
    /// `data`. Required.
    #[serde(rename = "data")]
    pub data: NArrayOfQueryWebSocketGrainDataFlow,
}

impl NQueryWebSocketGrainFlow {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryWebSocketGrainFlow",
            ));
        };
        ...
        let data = match doc.get("data") {
            Some(v) => Some(NArrayOfQueryWebSocketGrainDataFlow::decode(v)?),
            None => None,
        };
