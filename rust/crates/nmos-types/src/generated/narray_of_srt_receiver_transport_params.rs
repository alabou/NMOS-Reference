...
use crate::generated::nsrt_receiver_transport_params::NSrtReceiverTransportParams;
...
/// `NArrayOfSrtReceiverTransportParams`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfSrtReceiverTransportParams(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NSrtReceiverTransportParams>,
);

impl NArrayOfSrtReceiverTransportParams {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfSrtReceiverTransportParams",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NSrtReceiverTransportParams::decode(item)?);
        }
        Ok(Self(out))
    }
}
