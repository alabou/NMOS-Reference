...
use crate::generated::noutput::NOutput;
...
impl NArrayOfOutput {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data("expected array for NArrayOfOutput"));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NOutput::decode(item)?);
        }
        Ok(Self(out))
    }
}
