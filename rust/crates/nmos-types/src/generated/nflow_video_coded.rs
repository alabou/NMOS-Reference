...
pub struct NFlowVideoCoded {
    ...
    /// `sublevel`. Optional, so absent means the member was not present.
    #[serde(rename = "sublevel", skip_serializing_if = "Option::is_none")]
    pub sublevel: Option<EnumId>,
    /// `fbblevel`. Optional, so absent means the member was not present.
    #[serde(rename = "fbblevel", skip_serializing_if = "Option::is_none")]
    pub fbblevel: Option<EnumId>,
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NFlowVideoCoded",
            ));
        };
        ...
        let sublevel = match doc.get("sublevel") {
            Some(v) => Some(decode::enum_id(v)?),
            None => None,
        };
        let fbblevel = match doc.get("fbblevel") {
            Some(v) => Some(decode::enum_id(v)?),
            None => None,
        };
