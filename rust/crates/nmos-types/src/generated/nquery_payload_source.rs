...
use crate::generated::nquery_web_socket_grain_source::NQueryWebSocketGrainSource;
...
pub struct NQueryPayloadSource {
    /// `grain_type`. Required.
    ...
    /// `grain`. Required.
    #[serde(rename = "grain")]
    pub grain: NQueryWebSocketGrainSource,
}

impl NQueryPayloadSource {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryPayloadSource",
            ));
        };
        ...
        let grain = match doc.get("grain") {
            Some(v) => Some(NQueryWebSocketGrainSource::decode(v)?),
            None => None,
        };
