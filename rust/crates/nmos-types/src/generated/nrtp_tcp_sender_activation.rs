...
use crate::generated::narray_of_rtp_tcp_sender_transport_params::NArrayOfRtpTcpSenderTransportParams;
...
pub struct NRtpTcpSenderActivation {
    /// `receiver_id`. Optional, so absent means the member was not present.
    ...
    pub transport_params: Option<NArrayOfRtpTcpSenderTransportParams>,
}

impl NRtpTcpSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NRtpTcpSenderActivation",
            ));
        };

        let receiver_id = match doc.get("receiver_id") {
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfRtpTcpSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NRtpTcpSenderActivation"
                )));
            }
        }
