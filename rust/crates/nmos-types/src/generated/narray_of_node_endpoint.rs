...
use crate::generated::nnode_endpoint::NNodeEndpoint;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfNodeEndpoint",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NNodeEndpoint::decode(item)?);
        }
        Ok(Self(out))
    }
}
