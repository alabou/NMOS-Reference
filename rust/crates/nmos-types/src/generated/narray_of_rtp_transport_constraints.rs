...
use crate::generated::nrtp_transport_constraints::NRtpTransportConstraints;
...
/// `NArrayOfRtpTransportConstraints`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NArrayOfRtpTransportConstraints(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NRtpTransportConstraints>,
);

impl NArrayOfRtpTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NArrayOfRtpTransportConstraints",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NRtpTransportConstraints::decode(item)?);
        }
        Ok(Self(out))
    }
}
