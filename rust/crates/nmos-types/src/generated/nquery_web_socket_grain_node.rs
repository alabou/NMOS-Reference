...
use crate::generated::narray_of_query_web_socket_grain_data_node::NArrayOfQueryWebSocketGrainDataNode;
...
/// `NQueryWebSocketGrainNode`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NQueryWebSocketGrainNode {
    /// `type`. Required.
    #[serde(rename = "type")]
    pub r#type: String,
    /// `topic`. Required.
    ...
    /// `data`. Required.
    #[serde(rename = "data")]
    pub data: NArrayOfQueryWebSocketGrainDataNode,
}

impl NQueryWebSocketGrainNode {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryWebSocketGrainNode",
            ));
        };
        ...
        let data = match doc.get("data") {
            Some(v) => Some(NArrayOfQueryWebSocketGrainDataNode::decode(v)?),
            None => None,
        };
