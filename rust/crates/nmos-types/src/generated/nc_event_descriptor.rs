...
pub struct NcEventDescriptor {
    ...
    #[serde(rename = "name")]
    pub name: String,
    /// `eventDatatype`. Required.
    #[serde(rename = "eventDatatype")]
    pub event_data_type: String,
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NcEventDescriptor",
            ));
        };
        ...
        let event_data_type = match doc.get("eventDatatype") {
            Some(v) => decode::string(v)?,
            None => None,
        };
        ...
        let event_data_type = event_data_type
            .ok_or_else(|| Error::invalid_object("missing required member EventDataType"))?;
