...
impl NcCounter {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data("expected JSON object for NcCounter"));
        };
        ...
        let count = count.ok_or_else(|| Error::invalid_object("missing required member Count"))?;
