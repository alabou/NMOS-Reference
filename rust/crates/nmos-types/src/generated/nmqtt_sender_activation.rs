...
use crate::generated::narray_of_mqtt_sender_transport_params::NArrayOfMqttSenderTransportParams;
...
pub struct NMqttSenderActivation {
    ...
    pub transport_params: Option<NArrayOfMqttSenderTransportParams>,
}

impl NMqttSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NMqttSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfMqttSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NMqttSenderActivation"
                )));
            }
        }
