...
use crate::generated::narray_of_ndi_sender_transport_params::NArrayOfNdiSenderTransportParams;
...
pub struct NNdiSenderActivation {
    ...
    pub transport_params: Option<NArrayOfNdiSenderTransportParams>,
}

impl NNdiSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NNdiSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfNdiSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NNdiSenderActivation"
                )));
            }
        }
