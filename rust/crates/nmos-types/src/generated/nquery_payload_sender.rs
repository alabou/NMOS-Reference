...
use crate::generated::nquery_web_socket_grain_sender::NQueryWebSocketGrainSender;
...
pub struct NQueryPayloadSender {
    /// `grain_type`. Required.
    ...
    /// `grain`. Required.
    #[serde(rename = "grain")]
    pub grain: NQueryWebSocketGrainSender,
}

impl NQueryPayloadSender {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryPayloadSender",
            ));
        };
        ...
        let grain = match doc.get("grain") {
            Some(v) => Some(NQueryWebSocketGrainSender::decode(v)?),
            None => None,
        };
