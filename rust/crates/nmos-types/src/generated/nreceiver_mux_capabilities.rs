...
/// `NReceiverMuxCapabilities`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NReceiverMuxCapabilities {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NReceiverMuxCapabilities",
            ));
        };
        ...
        if let Some(v) = &media_types {
            validators::check_mux_media_types(v.iter().map(nmos_json::EnumId::as_str))?;
        }
