...
use crate::generated::nc_event_descriptor::NcEventDescriptor;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NcArrayOfEventDescriptor",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcEventDescriptor::decode(item)?);
        }
        Ok(Self(out))
    }
}
