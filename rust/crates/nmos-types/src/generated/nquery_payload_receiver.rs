...
use crate::generated::nquery_web_socket_grain_receiver::NQueryWebSocketGrainReceiver;
...
/// `NQueryPayloadReceiver`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NQueryPayloadReceiver {
    /// `grain_type`. Required.
    ...
    /// `grain`. Required.
    #[serde(rename = "grain")]
    pub grain: NQueryWebSocketGrainReceiver,
}

impl NQueryPayloadReceiver {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NQueryPayloadReceiver",
            ));
        };
        ...
        let grain = match doc.get("grain") {
            Some(v) => Some(NQueryWebSocketGrainReceiver::decode(v)?),
            None => None,
        };
