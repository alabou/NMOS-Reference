...
impl NArrayOfNode {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data("expected array for NArrayOfNode"));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NNode::decode(item)?);
        }
        Ok(Self(out))
    }
}
