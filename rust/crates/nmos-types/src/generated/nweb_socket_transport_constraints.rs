...
/// `NWebSocketTransportConstraints`.
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct NWebSocketTransportConstraints {
    /// `Constraints`, flattened into this type's own JSON scope.
    #[serde(flatten)]
    pub constraints: NTransportConstraints,
}

impl NWebSocketTransportConstraints {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NWebSocketTransportConstraints",
            ));
        };
