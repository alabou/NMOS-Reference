...
use crate::generated::narray_of_usb_sender_transport_params::NArrayOfUsbSenderTransportParams;
...
pub struct NUsbSenderActivation {
    ...
    pub transport_params: Option<NArrayOfUsbSenderTransportParams>,
}

impl NUsbSenderActivation {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        let Some(doc) = src.as_object() else {
            return Err(Error::invalid_data(
                "expected JSON object for NUsbSenderActivation",
            ));
        };
        ...
        let transport_params = match doc.get("transport_params") {
            Some(v) => Some(NArrayOfUsbSenderTransportParams::decode(v)?),
            None => None,
        };
        ...
        for prop in doc.keys() {
            if !KNOWN.contains(&prop.as_str()) {
                return Err(Error::invalid_data(format!(
                    "unknown property '{prop}' in sealed type NUsbSenderActivation"
                )));
            }
        }
