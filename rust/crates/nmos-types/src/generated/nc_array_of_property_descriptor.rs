...
use crate::generated::nc_property_descriptor::NcPropertyDescriptor;
...
/// `NcArrayOfPropertyDescriptor`: a list carried without a wrapping object.
///
/// `#[serde(transparent)]` so it encodes as its contents, with no extra nesting,
/// matching the Python type's encode.
#[derive(Debug, Clone, PartialEq, Serialize)]
#[serde(transparent)]
pub struct NcArrayOfPropertyDescriptor(
    /// The wrapped value, encoded with no enclosing object.
    pub Vec<NcPropertyDescriptor>,
);

impl NcArrayOfPropertyDescriptor {
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for NcArrayOfPropertyDescriptor",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(NcPropertyDescriptor::decode(item)?);
        }
        Ok(Self(out))
    }
}
