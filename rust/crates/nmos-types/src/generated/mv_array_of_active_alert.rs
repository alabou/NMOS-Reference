...
use crate::generated::mv_active_alert::MvActiveAlert;
    ...
    pub fn decode(src: &Value) -> Result<Self> {
        ...
        let Value::Array(items) = src else {
            return Err(Error::invalid_data(
                "expected array for MvArrayOfActiveAlert",
            ));
        };
        let mut out = Vec::with_capacity(items.len());
        for item in items {
            out.push(MvActiveAlert::decode(item)?);
        }
        Ok(Self(out))
    }
}
