...
if errorlevel 1 (
  >&2 echo start-node3.bat: invalid registration port "%RDS_REG_PORT%"
  set "EXIT_CODE=64"
  goto done
)
...
rem Absent SNX00003, the probe above falls through to the workspace tree,
rem which a standalone clone does not have. Say that plainly here rather
rem than letting the CA-bundle copy fail with a bare directory name.
if not exist "%CERTS%\ExampleRootCA.pem" (
  >&2 echo start-node3.bat: no Certificates tree at "%CERTS%"
  >&2 echo Set IPMX_CERT_ROOT to a Certificates tree carrying SNX00003, or
  >&2 echo drop the SNX00003 files into "%SCRIPT_DIR%Certificates\build.0".
  set "EXIT_CODE=66"
  goto done
)
...
copy /b "%CERTS%\ExampleRootCA.pem" + "%CERTS%\ExampleRootCA.ec.pem" "%CA%" >nul || (
  >&2 echo start-node3.bat: cannot build CA bundle from "%CERTS%"
  set "EXIT_CODE=1"
  goto done
)
...
call :find_python
if errorlevel 1 (
  >&2 echo start-node3.bat: Python 3.12 or newer was not found.
